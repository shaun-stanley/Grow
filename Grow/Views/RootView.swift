...
struct RootView: View {
    var body: some View {
        TabView {
            Tab("Today", systemImage: "leaf") { HomeTwinScreen() }
            Tab("Care", systemImage: "drop") { TodayScreen() }
            Tab("Capture", systemImage: "camera") { CaptureScreen() }
            Tab("Reels", systemImage: "play.rectangle") { ReelsScreen() }
            Tab("Dex", systemImage: "square.grid.2x2") { DexScreen() }
        }
...
struct HomeTwinScreen: View {
    ...
    private func plantFirst() {
        ...
        store.createGrow(speciesID: pick.id, nickname: "", system: .kratky)
    }
}
...
#Preview {
    RootView()
        .environment(PlantCatalogService())
        .environment(GrowStore(context: GrowModelContainer.shared.mainContext, catalog: PlantCatalogService()))
}
